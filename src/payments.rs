...
use crate::models::{client::payment::PaymentCreate, server::payment::{Card, Pix, Response, Status}};
...
impl Payments {
  ...
  pub async fn create(&self, data: PaymentCreate) -> Result<Response, String> {
    ...
    let res = client.post(format!("{}/api/{}/create", self.__api, if let PaymentCreate::Checkout(_) = data { "checkout" } else { "payment" }))
    ...
    match data {
      ...
      */
      PaymentCreate::Card(_) => Ok(Response::Card(Card {
        status: Status::Pending,
        payment_id:    response.clone()
                        .get("data")
                        .unwrap()
                        .get("payment_id")
                        .unwrap()
      ...
      Payment with PIX
      */
      PaymentCreate::Pix(_) => {Ok(Response::Pix(Pix {
        status: Status::Pending,
        payment_id: response.clone()
                      .get("data")
                      .unwrap()
      ...
      PaymentCreate::Checkout(_) => Ok(Response::Checkout(
        response
          .clone()
          .get("data")
          .unwrap()
  ...
  pub async fn obtain(&self, id: impl ToString) -> Result<Response, String> {
    ...
    let res = client.get(format!("{}/api/payment/get", self.__api))
      .header("Authorization-key", self.__auth.clone())
      .header("Content-Type", "application/json")
    ...
    let status: Status = crate::utils::status::transform(
      response
        .clone()
        .get("data")
        .unwrap()
